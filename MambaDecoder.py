...
from  VMamba.kernels.vmamba import CVSSDecoderBlock, AlignedMambaBlock
...
class MambaDecoder(nn.Module):
    ...
    def forward_up_features(self, inputs,CR):  # B, C, H, W
        if not self.deep_supervision:
            for inx, layer_up in enumerate(self.layers_up):
                if inx == 0:
                    x1 = inputs[3 - inx] 
                    cr1=CR[3 - inx] # B, 768, 14, 14
                    x2 = inputs[3 - inx -1]
                    cr2=CR[3 - inx -1] 
                    x1=x1+cr1  
                    x2=x2+cr2        # B, 768, 28, 28
                    ...
                    skip_feat = inputs[3 - inx] + CR[3 - inx]
                    skip_feat = skip_feat.permute(0, 2, 3, 1).contiguous()
                    x1 = y + skip_feat
                    feat_next = inputs[3 - inx - 1] + CR[3 - inx - 1]
                    x2 = feat_next.permute(0, 2, 3, 1).contiguous()
                    y = layer_up(x1, x2)
