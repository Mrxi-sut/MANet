...
class Data(Dataset):
    def __init__(self, root,mode='train'):
        ...
        # only include common image files (skip .DS_Store and other non-image files)
        try:
            lines = [f for f in os.listdir(gt_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    ...
    def __getitem__(self, idx):
        ...
        if rgb is None:
            raise FileNotFoundError(f"Failed to read RGB image: {rgbpath}")
        if t is None:
            raise FileNotFoundError(f"Failed to read thermal image: {tpath}")
        if mask is None:
            raise FileNotFoundError(f"Failed to read GT mask: {maskpath}")
